...
logger = logging.getLogger("FAIZ")

channel_layer = get_channel_layer()
_redis = redis.Redis(host = 'localhost', port = REDIS_PORT, db = 0) 

def handle_log(job, d, log_objects, logs):
    new_log = {"job":             job,
               "message":         d["msg"],
...
def send_data(active_job_ids, model_names, logs, progress, abort_ack):
    ...
    if data_exists:
        ...
        message = {'type': 'send_job_data', 'all_data': all_data}
        ...
        for job in active_jobs:
            ...
            while running_inner:
                ...
                if x is not None:
                    running_outer = True
                    d = json.loads(x)
                    if d["name"] == "STDERR" or d["name"] == "STDOUT":
                        handle_log(job, d, log_objects, logs)
                    elif d["name"] == "PROGRESS":
                        epoch = handle_progress(d, epoch, progress, job)
                    elif d["name"] == "ABORT":
                        abort_ack[job.id] = True
                        reloaded_job = Job.objects.get(id=job.id)
                        if (reloaded_job.status == JobStatus.PAUSING):
